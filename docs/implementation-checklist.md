...
> Status: **The live repository is preparing an Atlas-branded `xurgo-atlas` v0.5.0 local release candidate; published v0.4.0 remains the baseline until separate remote, tag, and publication gates complete**
>
> Current state:
...
| Package baseline | ✅ Local candidate | `package.json` reports version `0.5.0`; published `v0.4.0` remains the release baseline until external gates complete |
| Release posture | ✅ Local preparation | The backward-compatible read-only `atlas.artifact_registration_status` MCP addition is under local release-candidate validation; no publication is implied |
